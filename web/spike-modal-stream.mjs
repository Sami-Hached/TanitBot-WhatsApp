...
//
// Requires MODAL_STREAM_URL / MODAL_PROXY_TOKEN_ID / MODAL_PROXY_TOKEN_SECRET, and
...
for (const name of ["MODAL_STREAM_URL", "MODAL_PROXY_TOKEN_ID", "MODAL_PROXY_TOKEN_SECRET"]) {
  if (!process.env[name]) {
...
const res = await fetch(process.env.MODAL_STREAM_URL, {
  method: "POST",
  redirect: "manual",
  headers: {
